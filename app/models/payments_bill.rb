class PaymentsBill < ActiveRecord::Base
	belongs_to :user
	validates :service_name, :customer_name, :network, :mobile_number, :amount, :total, :bill_number, :user, presence: true
	...
	def bill_number
		...
		last_bill_number = PaymentsBill.today_bills.first.try(:bill_number)
		if last_bill_number.nil?
			bill_no = "P" + "#{Time.zone.now.strftime("%y%m%d")}-0001"
		else
	...
	def set_total
		self.total = self.amount + ((self.amount >= 100) ? 20 : 20) if self.amount
	end
end